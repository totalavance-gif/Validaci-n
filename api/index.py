...
def procesar():
    try:
        ...
        # ---- Datos Centrados en Cédula (X_center = 165pt)
        # Usamos un ancho de celda de 330pt (165*2) para que el centro sea 165
        w_cedula = pt_to_mm(330)
        pdf.set_font("SansBold", size=8)
        pdf.set_xy(0, pt_to_mm(792 - 638))
        pdf.cell(w_cedula, 0, rfc, align='C')

        pdf.set_font("Sans", size=6.5)
        pdf.set_xy(0, pt_to_mm(792 - 612))
        pdf.cell(w_cedula, 0, nombre_full, align='C')

        pdf.set_xy(0, pt_to_mm(792 - 595))
        pdf.cell(w_cedula, 0, f"idCIF: {idcif}", align='C')

        # ---- Lugar y Fecha (X_center = 364pt)
        pdf.set_font("SansBold", size=7.5)
        pdf.set_xy(0, pt_to_mm(792 - 683))
        pdf.cell(pt_to_mm(364*2), 0, lugar_fecha_txt, align='C')
        ...
        pdf.cell(pt_to_mm(350), 0, f"||1.1|{idcif}|{datetime.now().strftime('%Y-%m-%dT%H:%M:%S')}|{rfc}||")
