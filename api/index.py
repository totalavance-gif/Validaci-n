...
TAMANO_FUENTE = 39
TAMANO_SELLOS = 22  # Fuente más pequeña para los sellos digitales
...
# --- COORDENADAS: PLANTILLA 2 (NUEVO MAPEO) ---
# Tablas
P2_ORDEN = (113, 611)
...
def procesar_hojas(datos):
    ...
    cadena = f"||{fecha_sello}|{datos['rfc']}|CONSTANCIA DE SITUACION FISCAL|200001088888800351521||{generar_aleatorio(40)}="
    sello = f"{generar_aleatorio(80)}\n{generar_aleatorio(80)}="
    
    d2.text(P2_CADENA_ORIGINAL, cadena, fill="black", font=font_sello)
    d2.text(P2_SELLO_DIGITAL, sello, fill="black", font=font_sello)
    d2.rectangle([P2_QR, (P2_QR[0]+524, P2_QR[1]+524)], fill="black")

    # Por ahora devolvemos la P2 para que la veas, luego podemos unirlas en un PDF
    img_io = io.BytesIO()
    img2.convert('RGB').save(img_io, 'PNG')
    img_io.seek(0)
    return img_io
