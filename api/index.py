...
# --- COORDENADAS FINALES (Imagen 2550x3300) ---
# QR ajustado: X=140 (-3mm), Y=631 (-2mm)
COORD_RFC = (635, 545)
...
def procesar_imagen_servidor(datos):
    # Cargar plantilla desde la raíz del proyecto
    base_path = os.path.join(os.path.dirname(__file__), '..', 'plantilla.png')
    img = Image.open(base_path).convert('RGBA')
    draw = ImageDraw.Draw(img)
    
    # --- CONFIGURACIÓN DE FUENTES (Tamaño 39) ---
    try:
        # Fuentes estándar en servidores Linux (Vercel)
        ...
        # Fallback si no detecta las fuentes del sistema
        font_normal = ImageFont.load_default(size=39)
        font_bold = ImageFont.load_default(size=39)

    # 1. Dibujar RFC, Nombre e idCIF (Estilo Normal)
    ...
    # Sin acentos para evitar errores de renderizado en servidor
...
def procesar():
    ...
    # Formato de fecha larga: a dd de mm del aaaa
